...
## 第 39 项限速修复（2026-09-12）

第 39 项现在仅管理**明确选定网卡的、带非零 handle 的现有根 FQ 每流上限**，不再切换 BBR、压缩 TCP 缓冲区或对所有接口应用规则。它包含所选接口的 TCP/UDP 出站流量，并非按账号或总带宽限速。为保留原队列，遇到 CAKE、HTB、mq、FQ_CoDel、内核默认 handle `0:` 等结构会拒绝自动覆盖；本功能不会自动安装或转换队列。

如果 `tc -j qdisc show dev eth0` 显示根 FQ 的 handle 是 `0:`，请先确认要迁移现有队列，再手动建立带非零 handle 的根 FQ；脚本不会为了限速自动替换它，因为这种替换可能丢失原队列参数。
...
- 限速**不自动跨重启或网卡重建恢复**。重启后的旧记录可通过子菜单 6 删除，随后重新选择接口启用。接口在同次启动内重建或被其他工具修改时停止自动恢复，需要核对实际队列。
