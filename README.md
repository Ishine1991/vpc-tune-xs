...
交互操作 **39 → 1/2** 遇到 IFB 不可用时，可直接按编号选择已安装的候选内核。确认救援入口和风险后，脚本备份 GRUB 环境块、设置并读回校验**下一次试启动**，然后单独询问是否立即重启（默认否）。选择不立即重启仍会保留下次启动安排；可用 `grub-editenv /boot/grub/grubenv unset next_entry` 取消。试启动阶段不会安装或删除内核、修改永久默认内核，也不会把尚未生效的输入速率保存为成功策略。
...
试启动助手只处理可识别的 Debian/Ubuntu 标准 GRUB 菜单、普通磁盘上的 ext2/3/4 环境块；已有 `next_entry`、LVM/RAID/加密、其他文件系统或未知布局会拒绝自动处理。一次性启动不保证失败时自动重启回退，必须有可用的云平台控制台/救援入口。开机恢复和非交互运行永远不会弹出此助手或执行重启。依据 [Debian grub-reboot 文档](https://manpages.debian.org/trixie/grub2-common/grub-reboot.8.en.html)。
