...
第 39 项及开机恢复会在修改出站队列前检查 IFB，必要时自动执行 `modprobe ifb numifbs=0`；兼容工具位于 `/usr/sbin` 或 `/sbin` 而不在 PATH 的情况。若当前内核未编译 IFB 或模块加载失败，会停止并列出本机具备 IFB 配置、启动文件及模块的候选内核（不代表已通过启动验证）。不会自动安装/切换内核、修改 GRUB 或重启。IFB 创建失败且确认没有设备或入站队列遗留时，会自动清理空失败记录以便重试；归属不明的资源仍保留保护。
