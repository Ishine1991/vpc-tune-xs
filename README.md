...
第 39 项及开机恢复会在修改出站队列前检查 IFB，必要时自动执行 `modprobe ifb numifbs=0`；兼容工具位于 `/usr/sbin` 或 `/sbin` 而不在 PATH 的情况。若当前内核未编译 IFB 或模块加载失败，会列出本机具备 IFB 配置、启动文件及模块的候选内核（不代表已通过启动验证）。IFB 创建失败且确认没有设备或入站队列遗留时，会自动清理空失败记录以便重试；归属不明的资源仍保留保护。

交互操作 **39 → 1/2** 遇到 IFB 不可用时，可直接按编号选择已安装的候选内核。普通 ext2/3/4 磁盘在确认救援入口后，备份 GRUB 环境块、设置并读回校验**下一次试启动**，然后单独询问是否立即重启（默认否）。选择不立即重启仍会保留下次启动安排；可用 `grub-editenv /boot/grub/grubenv unset next_entry` 取消。试启动阶段不会安装或删除内核、修改永久默认内核，也不会把尚未生效的输入速率保存为成功策略。XFS、Btrfs、LVM 等不能安全改写环境块的布局，会改为在确认救援入口后直接把所选内核设为永久默认，并保留原菜单备份；这种情况下启动失败不会自动回到旧内核。

重启成功后，在 **39 → 1/2** 设置双向限速成功，会询问是否将当前内核设为**永久默认**；也可单独选择 **39 → 8**。仅允许固定当前正在运行、双向队列校验通过且磁盘启动文件/IFB 模块完整的内核。确认后通过 `/etc/default/grub.d/zz-net-tcp-tune-default.cfg` 指定默认项，先备份原配置，再生成并验证新菜单，最后替换 `/boot/grub/grub.cfg`；生成或验证失败会尝试撤销本次覆盖配置，不替换原启动菜单。备份保存在输出提示的 `/boot/grub/pacing-default-backup.*` 目录。固定默认无需再重启，正常后续启动即可使用该内核并按已保存策略恢复限速。旧内核保留；固定到具体版本后，新安装内核不会自动成为默认，需验证后重新设置。

试启动只处理可识别的 Debian/Ubuntu 标准 GRUB 菜单，以及普通磁盘上的 ext2/3/4 环境块。已有 `next_entry` 时不覆盖。其他文件系统或 LVM/RAID/加密布局不写入环境块，可在明确确认后改永久默认。一次性启动不保证失败时自动重启回退；直接改永久默认时，失败更不会自动回到旧内核。两种方式都必须有可用的云平台控制台/救援入口。开机恢复和非交互运行永远不会弹出此助手或执行重启。依据 [GRUB 环境块说明](https://www.gnu.org/software/grub/manual/grub/html_node/Environment-block.html)和 [Debian grub-reboot 文档](https://manpages.debian.org/trixie/grub2-common/grub-reboot.8.en.html)。

选项 6 会同时清理上次启动的出站和入站记录；存在当前启动的记录或损坏记录时不会删除任何记录。创建入站队列后保存归属失败，会利用当前进程已确认的信息立即回滚。若在创建成功与记录保存之间被强制终止，则无法可靠确认队列归属，仍需人工核对；不会猜测归属并删除队列。
...
virtio 等网卡常见根 `mq handle 0:`、叶子 `parent :1/:2`。选项 **1 会自动迁移** 成可定址 handle（可能短暂丢包），不必再先走选项 7。半完成迁移（根已非零、叶子仍是 `handle 0:`）也会自动修。迁移失败时**不会**删除根队列。选项 7 仍可用于手动迁移/修复。

`mq + FQ_CoDel` 转换要求全部直属发送叶子都是 FQ_CoDel、没有过滤器，且原参数可安全重建；未知参数、旧限速记录或损坏记录会阻止转换。原参数写入 `.migration-网卡-时间戳` 备份；中途失败会尝试恢复原 FQ_CoDel 参数并读回验证，保留 mq，零 handle 可能变为可定址 handle。成功后关闭限速**只清除每流上限，不切回 FQ_CoDel**。重启后若内核重新创建 FQ_CoDel 叶子，已保存的限速策略会重新转换和限速；不修改全局 `default_qdisc`。
...
一键安装 XanMod 内核，启用 BBR v3 拥塞控制，集成主菜单 1–39 项实用工具，优化你的 VPS 服务器。

> **版本**: 脚本 2.4 / 工具集（Snell v5.0.1 + 第 39 项 FQ 每流限速）  
...
本脚本主菜单为 **1–39**（另有 99 卸载），涵盖内核优化、网络加速、代理部署、系统管理等。
