...
QEMU/virtio 等多队列网卡常见根 `mq handle 0:`、叶子 `FQ parent :1/:2`。这种零 handle 布局无法被 `tc qdisc change` 定址，直接选 39→1 会失败。请先 **39 → 7** 按编号选择网卡并迁移，再 **39 → 1** 输入 `20M`（不是 `20`），同样按编号选网卡。较新内核（6.7+）的 FQ 会带 `bands`/`priomap`/`weights`，迁移会原样重建，不再因这些默认优先级带报错。
