...
交互操作 **39 → 1/2** 遇到 IFB 不可用时，可直接按编号选择已安装的候选内核。确认救援入口和风险后，脚本备份 GRUB 环境块、设置并读回校验**下一次试启动**，然后单独询问是否立即重启（默认否）。选择不立即重启仍会保留下次启动安排；可用 `grub-editenv /boot/grub/grubenv unset next_entry` 取消。不会安装或删除内核、修改永久默认内核，也不会把尚未生效的输入速率保存为成功策略。重启成功后需再次进入 39 设置限速，永久默认内核仍需另行确认。
