...
第 39 项现在仅管理**明确选定网卡的现有根 FQ 每流上限**，不再切换 BBR、压缩 TCP 缓冲区或对所有接口应用规则。它包含所选接口的 TCP/UDP 出站流量，并非按账号或总带宽限速。为保留原队列，遇到 CAKE、HTB、mq、FQ_CoDel 等其他结构会拒绝自动覆盖；本功能不会自动安装或转换队列。
