...
第 39 项管理**选定网卡的 FQ 出站每流上限**（TCP 和 UDP 都会被限制，入站不管，也不是整卡总带宽）。支持单根 FQ，也支持 `mq + FQ` 叶子。保留根 `mq`，对全部直属 FQ 发送叶子设置相同上限；任一叶子失败会尝试整体回滚。不切换 BBR，不改 TCP 缓冲区。遇到 CAKE、HTB、FQ_CoDel 或混合叶子会拒绝覆盖。
